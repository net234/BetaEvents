...
void evHandlerUdp::handle() {
  ...
  String bStr;
  if (aStr.startsWith("EVENT")) {
    bStr = grabFromStringUntil(aStr, '\t');     // EVENT xxxx
    rxHeader = grabFromStringUntil(bStr, ' ');  // header    bstr=num trame
    rxNode = grabFromStringUntil(aStr, '\t');   // node
  } else if (aStr.startsWith("{\"TRAME\"")) {
    rxHeader = "TRAME";
    grabFromStringUntil(aStr, ':');
    bStr = grabFromStringUntil(aStr, ",\"");  // bstr=num trame   astr = start of nodename
    rxNode = grabFromStringUntil(aStr, "\"");
    //D_println(rxNode);
    //D_println(bStr);
